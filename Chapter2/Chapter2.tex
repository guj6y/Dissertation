...
%        Facultative Micropredation                  & No            &0\%\\
        \bottomrule
    \end{tabular}
    \caption[Summary of empirical link types]{This table describes how link
        types in the empirical data were used to construct food webs. A ``Yes''
        under the ``Parasitic?'' column indicates that we defined the consumer
        in such links to be a parasite; an ``N/A'' in the same column indicates
        that we excluded that link type from our analysis. Link types marked by
        * denote concomittant links; we omit these from our analysis because
        they are well predicted by a simple structural algorithm as they arise
        solely from the interactions of two other organisms. See
        \cite*{Hechinger2011a} for complete descriptions of all link types.
...
\subsection{Properties of Parasites} We calculated common and more novel
properties of nodes in an attempt to find a structural trophic fingerprint of
parasitism. The properties calculated include traditional local properties of
ecological interest (vulnerability, generality, and trophic level, represented
for a node $i$ by $v_i$, $g_i$, and $T_i$, respectively) and 9 network
theoretic properties not commonly found in ecological studies. These more novel
properties emphasize global network properties associated with betweenness and
eigenvalues of the web's adjacency matrix and also node neighborhood properties
including nodes within 2 degrees of a node.
...
\begin{sidewaysfigure}       
    ...
    \includegraphics[width=\linewidth]%
            {\DissertationDir/Chapter2/figures/allPropsDistMaxLinkage.png}
            \caption[Community properties of free livers and parasites in
            aggregated webs]%
            {This figure shows the differences between the carnivore average
        ...
        corrected to control the family-wise error rate at each agglomeration
...
We analyzed the importance of the entire set of variables by training 1000
binary classification trees with all predictors, with each predictor removed
(figures \ref{fig:initialClassResults-a}, \ref{fig:initialClassResults-c}) and
with each predictor in isolation (figures \ref{fig:initialClassResults-b},
\ref{fig:initialClassResults-d}). We measured overall classification error as
the fraction of the error in a random model.
