    ...
    for ii = 1:nWebs
        ...
        for jj = idList'
            ...
            binj = nodeData.SpeciesID==jj;
            minStage = min(nodeData.StageID(binj));
            binMin = nodeData.StageID==minStage;

            bodySizes(jj) = mean(nodeData.BodySize_g_((binj)&(binMin)));

            biomasses(jj) = sum(nodeData.Biomass_kg_ha_(binj),'omitnan');
            if sum(binj) > 1
                if isnan(bodySizes(jj))&&sum(isfinite(nodeData.Biomass_kg_ha_(binj))>0)&&~(sum(isnan(nodeData.BodySize_g_((binj)&(binMin))))==sum((binj)&(binMin)))
                    bodySizes(jj) = nodeData.BodySize_g_((binj)&...
                        (nodeData.Biomass_kg_ha_==...
                        max(...
                        nodeData.Biomass_kg_ha_(binj&(isfinite(nodeData.BodySize_g_)))...
                        )...
                        )&isfinite(nodeData.BodySize_g_));
                end
        ...
        binOldAutoCell{ii}(nodeData.SpeciesID(~cellfun('isempty',strfind(nodeData.Feeding,'autotroph'))))=1;
        binOldDetCell{ii}(nodeData.SpeciesID(~cellfun('isempty',strfind(nodeData.OrganismalGroup,'detritus'))))=1;
        ...
        while ~trophicWeb
            ...
            for kk = I'
                ...
                bodySizes(equivClasskk(end)) = geomean(equivBodySizes(okEquivBodySizes));

                biomasses(equivClasskk(end)) = sum(biomasses(equivClasskk),'omitnan');
